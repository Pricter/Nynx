...
void _darray_destroy(void* array) {
    ...
    nfree(header, total_size, MEMORY_TAG_DARRAY);
}
...
void* _darray_resize(void* array) {
    ...
    ncopy_memory(temp, array, length * stride);
...
void* _darray_pop_at(void* array, u64 index, void* dest) {
    ...
    if(index >= length) {
        NERROR("Index outside the bounds of this array! Length: %i, index: %i", length, index);
        return array;
    }
...
void* _darray_insert_at(void* array, u64 index, void* value_ptr) {
    ...
    ncopy_memory((void*)(addr + (index * stride)), value_ptr, stride);
